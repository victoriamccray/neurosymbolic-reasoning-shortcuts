...
`slides/deck.html` is a 31 slide reveal.js deck in eight sections: Puzzle,
...
It is a single self-contained file. Every stylesheet, script and image is
inlined, so it needs no other file, no build step and no network access. Open
it from anywhere, including straight out of a ZIP archive.

```
# macOS / Linux
open slides/deck.html
...
`slides/assets/entangled_confusion.png` is generated by the notebook. The three
files under `slides/assets/` are the original figures, kept for provenance; the
deck itself embeds them, so it does not read from that directory.

`reveal.js/` vendors [reveal.js](https://github.com/hakimel/reveal.js) v6 under
the MIT licence. The deck inlines reveal.js v6, whose copyright banner is
retained in the file; the full licence text is at
`slides/reveal.js-LICENSE.txt`.
